...
jQuery(window).load(function() {
	...
	$('#selCountryCodes').on('change', function (e) {
	    var optionSelected = $("option:selected", this);
	    var valueSelected = this.value;
	    
	    $('#spnPrefixCode').html('+' + valueSelected);
	...
	$('#btnSubmit').on('click', function (e) {
	    ...
	    if (email !== '') {
	        $.getJSON("/Services/WebviewServices.svc/SubmitAndroidKitcut?email=" + email, function (data) {
	            if (data == 0) {
	                //  Failed
	    ...
	    if (email !== '') {
	        $.getJSON("/Services/WebviewServices.svc/SubmitAndroidKitcut?email=" + email, function (data) {
	            if (data == 0) {
	                ...
	                //  Success
	                $('#formSignupMobile').hide();
	                $('#formThanksMobile').show();
	            }
	        });
	    }
	    else {
...
function initMobilePause() {
	var playerAPI = jQuery('#video').data('mediaelementplayer');
	if(playerAPI) {
...
function initLoader() {
	...
	win.on('load', function() {
		jQuery('body').removeClass(loadingClass);
	})
}
